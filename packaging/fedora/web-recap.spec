...
* Wed Jun 24 2026 Ferran Fontcuberta Figueràs <ferran@fompi.net> - 0.5.0-1
...
- Added reference documentation for the internal on-disk data stores of Chrome, Firefox, and Safari (`docs/{chrome,firefox,safari}-internal-browser-data-stores.md`). Each document was verified column-by-column against real browser profiles with SQLite 3 and covers: the full history schema with every table, index, trigger (and an explicit note that none of the ~48 profile databases define any views); accurate Graphviz-generated ER diagrams (`docs/*_schema_erd.png`) regenerated from the live schema; a complete inventory of every navigation-related store (history, favicons, bookmarks, downloads, cookies, autofill, search engines, permissions, top sites, omnibox shortcuts); column-level descriptions decoding the cryptic fields (transition/visit_type codes, the three distinct timestamp epochs, status flags); enriched "real-world" example queries (verified to run, joining and `ATTACH`-ing across databases); and where open tabs, tab groups, reading lists, and synced/cross-device tabs live (SNSS, mozLz4, plist, and Sync LevelDB formats) with runnable extraction examples including a stdlib-only mozLz4 decoder.
- Added `docs/browser-data-stores-comparison.md`, a cross-browser comparison and normalization guide covering: what data is common to all three browsers (history core, navigation chains, visit type, bookmarks, session restore, downloads); the three incompatible timestamp epochs with conversion formulas; a unified `norm_visits` SQL schema with per-browser extraction queries (Chrome, Safari, Firefox) that map each browser's fields to a single normalized model; and a full catalog of unique-per-browser stores (Chrome Journeys/ML clusters, Safari visit histograms and plist tab groups, Firefox frecency and per-page engagement metrics) plus an "almost-common" comparison table for concepts that exist in multiple browsers but cannot be unified.
